// This file is used to used to generate trials from a STAIRCASE design

// Note 1:
// when using balanceFactor, we check whether trial is empty before trial.next
// but in staircase we first we first call trial.next(bool respYes) then check whether tiral is empty

// Note 2:
// it allows you to grab a factor not listed in the parameter files (e.g. trial.getCurrent().first["notExist"];
// but that may double your trials
...
//BalanceFactor<double> trial; //if using staircase: 
...
string parametersFile_directory = experiment_directory + "/parameters_template3_staircase.txt";
// response file headers
string responseFile_headers = "subjName\tIOD\tblockN\ttrialN\tdisplayDistance\tstdSize\tcompSize\tresp_stdDeeper\tID\treversals\tascending\tRT";
...
void initStreams()
{
	...
	string responseFileName = experiment_directory +"/"+ subjectName  + "_staircase.txt";
	// Principal streams files
	if (util::fileExists(experiment_directory +"/"+subjectName+"_staircase.txt") && subjectName != "junk")
	{
		string error_on_file_io = experiment_directory+"/"+subjectName+"_staircase.txt" + string(" already exists");
	...
	if(!trial_prepared){
		...
		if (currentStage == constantPresent) { 
			...
			if (training) {
				...
				size_std = trial.getCurrent().first["stdsize"];
				//size_test = trial.getCurrent().first["compsize"];
				size_comp = trial.getCurrent().second->getCurrentStaircase()->getState();
